...
**/
    ...
    virtual void Update(const OVRFW::ovrApplFrameIn& in) override {
        ...
        // oculus_reader
        const OVR::Matrix4f headPoseMatrix = OVR::Matrix4f(in.HeadPose);
        OVR::Matrix4f LeftRemoteTransform;
        OVR::Matrix4f RightRemoteTransform;
        if (in.LeftRemoteTracked) {
            OVR::Matrix4f::Multiply(&LeftRemoteTransform,
                                    headPoseMatrix.Inverted(), OVR::Matrix4f(in.LeftRemotePose));
        }
        if (in.RightRemoteTracked) {
            OVR::Matrix4f::Multiply(&RightRemoteTransform,
                                    headPoseMatrix.Inverted(), OVR::Matrix4f(in.RightRemotePose));
        }
        bool bA = buttonA.currentState == XR_TRUE;
        bool bB = buttonB.currentState == XR_TRUE;
        bool bX = buttonX.currentState == XR_TRUE;
        bool bY = buttonY.currentState == XR_TRUE;
        RemoteState remoteState{triggerTouchL_, triggerTouchR_, bA, bB, bX, bY, LeftRemoteTransform, RightRemoteTransform};
        auto remotes_str = remotes_to_string(remoteState, in);
